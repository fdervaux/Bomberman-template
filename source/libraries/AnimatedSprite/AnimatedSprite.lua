...
function AnimatedSprite:updateAnimation()
    if (self._enabled) then
        ...
        if ((self._ticks - self._previousTicks) >= self.states[self.currentState].tickStep) then
            ...
            if (type(loop) == "number" and loop <= loopsFinished or type(loop) == "boolean" and not loop and
                loopsFinished >= 1 or self.forcedSwitchOnLoop == loopsFinished) then
